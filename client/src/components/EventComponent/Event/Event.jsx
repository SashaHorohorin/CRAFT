...
const Event = ({event}) => {
    ...
    const getDate = (date) => {
        let d = new Date(date);
        let time = `${d.getDate() < 10 ? `0${d.getDate()}` : d.getDate()}.${d.getMonth() < 10 ? `0${d.getMonth()}` : d.getMonth()}`;
        return time;
    };
        ...
        <div className="events__event event">
            <div className="event__content content-event">
                ...
                <div className="content-event__time time-event">
                    <div className="time-event__time">{getTime(event.createdDate)}</div>
                    <div className="time-event__date">{getDateYear(event.createdDate)}</div>
                </div>
            </div>
            <div className="event__info info-event">
                <div className="info-event__left left-info">
                    ...
                    <div className="left-info__start">{event.smallTitle}</div>
                </div>
                <div className="info-event__right right-info">
                    <div className="right-info__date">{`${getDate(event.eventDate)}`}</div>
                    <div className="right-info__label">{event.textUnderDate}</div>
                </div>
            </div>
        </div>
        <div
